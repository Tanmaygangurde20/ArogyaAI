...
const Dropout = () => {
  ...
  return (
    <div className="min-h-screen pt-24 px-4 lg:px-8 max-w-3xl mx-auto">
      <h1 className="text-4xl font-bold mb-4 text-center text-purple-700">Dropout Risk Predictor</h1>
      <p className="text-center text-gray-600 mb-6">
        This tool predicts whether a child is likely to drop out from vaccination based on travel time, age, education level,
        and dose delays. The model helps in early intervention for high-risk cases.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-xl shadow">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="age" value={form.age} onChange={handleChange} className="w-full border rounded px-3 py-2" required />
          </div>
          <div>
        ...
        <div>
          <label className="block font-semibold mb-1">Parent Education</label>
          <select name="parent_education" value={form.parent_education} onChange={handleChange} className="w-full border rounded px-3 py-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block font-semibold mb-1">Dose 1 Date</label>
            <input type="date" name="dose1_date" value={form.dose1_date} onChange={handleChange} className="w-full border rounded px-3 py-2" required />
          </div>
          <div>
            <label className="block font-semibold mb-1">Dose 2 Date</label>
            <input type="date" name="dose2_date" value={form.dose2_date} onChange={handleChange} className="w-full border rounded px-3 py-2" required />
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block font-semibold mb-1">Distance to Center (km)</label>
            <input type="number" step="0.1" name="distance_to_center" value={form.distance_to_center} onChange={handleChange} className="w-full border rounded px-3 py-2" required />
          </div>
          <div>
            <label className="block font-semibold mb-1">Delay Days</label>
            <input type="number" name="delay_days" value={form.delay_days} onChange={handleChange} className="w-full border rounded px-3 py-2" required />
          </div>
        </div>
      ...
      {result && (
        <div className="mt-8 bg-purple-50 p-6 rounded-xl shadow text-center">
          <h2 className="text-2xl font-bold mb-2 text-purple-700">Prediction Result</h2>
